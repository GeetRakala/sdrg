...
std::tuple<int, double, std::vector<int>> ActivatedDijkstra( const BGLGraph& bglGraph, int start, int end, bool findToEndNode, bool debugDijkstra) {
  ...
  while (!min_heap.empty()) {
    ...
    if (findToEndNode && curr_node == end && graph[curr_node].status == NodeStatus::Active) {
      std::vector<int> path;
    ...
    for (boost::tie(ei, ei_end) = out_edges(curr_node, graph); ei != ei_end; ++ei) {
      ...
      if (findToEndNode) {
      // Skip this neighbor if it's an active node and not the end node
      if (graph[neighbor].status == NodeStatus::Active && neighbor != end) {
        continue;
      }
      }
